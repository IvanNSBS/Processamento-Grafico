...
void readFile(Scene *scene, const std::string &filename, std::string &newfile){
    ...
    string di = "Di";
    // scene->add( new Mesh( vec3(0.0, 0.18, 0), vec3(0.3, 0.3, 0.3), vec3(34.6f, -50.1f, -4.56f), findMat(mlist, name1 ), "./monkey_smooth.obj", true));
    ...
    scene->add( new Mesh( vec3(0, 0, 0), vec3(1,1,1), vec3(0, -50.1f,0 ), findMat(mlist, di ), "../objs/monkey.obj", true));
...
{ 
    //Para compilar: g++ -o r.exe -std=c++14 -pthread lodepng.cpp main.cpp
